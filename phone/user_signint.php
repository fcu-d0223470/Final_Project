<?php 
    ...
    if(mysqli_num_rows($check_user)==0){
		...
		$output_result=array('user'=>$user_output,'have'=>$have_output,'task'=>$task_output,'story'=>$story_output);
	    //	print(json_encode(array('user'=>$user_output,'have'=>$have_output,'task'=>$task_output,'story'=>$story_output)))
		//print(json_encode($output_result,JSON_UNESCAPED_UNICODE));
            }else{
				$have_output[]=array('have_id'=>'');
				$task_output[]=array('task_name'=>'');
				$story_output[]=array('story_name'=>'');
				//$output_result=array('user'=>$user_output,'have'=>'null','task'=>'null','story'=>'null');
				$output_result=array('user'=>$user_output,'have'=>$have_output,'task'=>$task_output,'story'=>$story_output);
				//print(json_encode($output_result,JSON_UNESCAPED_UNICODE));
			}   
			print(json_encode($output_result,JSON_UNESCAPED_UNICODE));
